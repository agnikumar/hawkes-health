...
for x = 1:num_units % total number of units
    ...
    %half = ceil(lx/2);
    half = ceil(lx); % train on all data
    ...
    alg.rho = 0.1;
    alg.inner = 10; % perviously, 5 or 6
    alg.thres = 1e-5;
    alg.Tmax = [];
    alg.storeErr = 0;
    alg.storeLL = 0;
...
writematrix(data, "data/univariate_traintest_10runs_30units_all.csv"); % mean rows followed by std rows (LL, A, mu)