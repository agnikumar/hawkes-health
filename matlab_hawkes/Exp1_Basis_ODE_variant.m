function model = Exp1_Basis_ODE_variant(Seqs, model, alg)
                        ...
                        if ~isempty(pij) && sum(pij(:))>0
                            for j = 1:length(uj)
                                ...
                                if n == alg.inner
                                    ...
                                    %Nums = min([ceil(dt(j)/model.dt), size(DM,1)]) + 1; % remove later
                                    DM(Nums,:) = DM(Nums,:) + ODE_pij(j,:);
                                    %Nums = Nums - 1;
                                    %DM(Nums,:) = min(DM(Nums,:) + ODE_pij(j,:), 500); % remove later
                                end
                                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            end
                        end
