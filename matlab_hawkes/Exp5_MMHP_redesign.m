...
for n = 1:nTest
    for i = 1:nSeg
        ...
        part_seqs_train = struct();
        for u = 1:U
            train_time_vect = seqs_train(u).Time();
            train_mark_vect = seqs_train(u).Mark();
            part = ceil(i*(length(train_time_vect))/10);
            part_seqs_train(u).Time = train_time_vect(1:part);
            part_seqs_train(u).Mark = train_mark_vect(1:part);
            part_seqs_train(u).Start = 0;
            part_seqs_train(u).Stop = max(part_seqs_train(u).Time());
        end
        
        model.A = rand(D,U,D)./(U*D^2); % using Initialization_Basis
        model.mu = rand(D,U)./D; % using Initialization_Basis
        
        output = Learning_MLE_Basis(part_seqs_train, model, alg);
